...
void ModeleCplex::allocVar(Instance * ins)
{
	...
	//_x[i][j] = 1 si le véhicule passe par l'arc (i,j) et 0 sinon 
...
void ModeleCplex::creationCtr(Instance * ins)
{
    ...
    //--------------------------------------------------------------------------------------------
    //ctr 1 et ctr 2 : Bornes inf de la fonction objectif
    ...
    { 
        ...
        {
			if(i!=j)
				expr += _x[i][j]*ins->_matrixDistVehicule[i][j];
        } 
    }
    ...
    //--------------------------------------------------------------------------------------------
    //ctr 3 : tous les clients non drone éligible sont servis par le véhicule
    ...
    //--------------------------------------------------------------------------------------------
    //ctr 4 et ctr5 : chaque client est servi sois par le véhicule ou par un drone exactement une fois
    ...
    //--------------------------------------------------------------------------------------------
    //ctr 6 : le véhicule part du dépôt au plus une fois
   
    for(int j = 0; j < N; ++j)
    { 
		expr += _x[0][ins->_listeClient[j]];

    }
	_mod->add(expr <= 1);
	//_mod->add(expr <= NB_VEH);
	//expr.clear();
	
	
    //--------------------------------------------------------------------------------------------
    //ctr 7 : contrainte de flow
    ...
    { 
        ...
        while((j < V) && (k < V))
		{
			...
			if(ins->_listeNode[k]->get_id() != ins->_listeClient[i])
				expr1 += _x[ins->_listeNode[k]->get_id()][ins->_listeClient[i]];
...
bool ModeleCplex::separateLazy(Instance *ins, const IloNumArray2 xSol, const IloNumArray zSol, IloExprArray lhs, IloNumArray rhs){
	...
	while (((cible=chooseNotVisited(G)) != NULL)){
		...
		contienDepot = false;
		while(!cible->listeSucc.empty() && !stop){
